...
public class MockMilvusController {
    ...
    public ResponseEntity<?> saveMilvus(){
        ...
        try {
            return new ResponseEntity<>(milvusService.insertData(1l, list, collectionName), HttpStatus.OK);
        } catch (Exception e) {
    ...
    public ResponseEntity<?> getMilvusById(@PathVariable("id") long id){
        String collectionName = "test2";
        try{
            return new ResponseEntity<>(milvusService.getData(id, collectionName), HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
